...
| O-11 | **No slot-delete path in the UI.** Three manual slots; a new run must bind to one; after three saved runs NEW GAME is refused with no way to free a slot from the menu. Needs a slot picker or an overwrite confirmation dispatching a command. | Lead UX | Before the Phase 6 playtest |
| O-12 | **`RecordedPercent` is never computed.** The pause summary and the save header render a value nothing produces. Either derive it from `WorldProgress` or drop the row until progression has a real percentage. | Lead Gameplay | Phase 6 |
