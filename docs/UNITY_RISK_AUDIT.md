...
| Input (`VardholmControls`) | **PARTIALLY RESOLVED — was HIGH RISK** — the C# API misuse is fixed (`expectedControlType` → `expectedControlLayout`, CS1739, caught by the compiler). The **binding strings themselves remain unverified**: `"2DVector(mode=2)"`, `<Keyboard>/w`, the processor strings. Those are parsed at runtime, so a typo still compiles and produces an action that silently never fires. Only pressing a key proves them. |
| ~~Input (original assessment)~~ | ~~HIGH RISK~~ | Actions are built **in code**, and the composite bindings use string syntax: `AddCompositeBinding("2DVector(mode=2)")` with `<Keyboard>/w` style paths and `processors:` strings. These are parsed at runtime, **not checked by the compiler**. A typo compiles cleanly and produces an action that silently never fires. This is the single most likely "compiles but does not work" area in the project. |
