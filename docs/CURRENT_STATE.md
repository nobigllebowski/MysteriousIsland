...
| Tests (310 EditMode cases, 30 PlayMode) | 22 | 7,117 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 340 tests written (310 EditMode + 30 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
