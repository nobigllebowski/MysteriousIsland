...
| Tests (393 EditMode cases, 35 PlayMode) | 27 | 9,410 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 428 tests written (393 EditMode + 35 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
