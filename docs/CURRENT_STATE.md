...
| Tests (387 EditMode cases, 30 PlayMode) | 26 | 8,924 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 417 tests written (387 EditMode + 30 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
