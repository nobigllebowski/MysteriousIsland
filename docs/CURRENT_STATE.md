...
| Tests (372 EditMode cases, 30 PlayMode) | 24 | 8,301 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 402 tests written (372 EditMode + 30 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
