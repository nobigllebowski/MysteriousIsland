...
**Updated:** 2026-09-14 · **Commit:** `f3a00e1` · **Branch:** `claude/keen-darwin-frw656`
...
| Documentation | 16 | — | 9 predate the ADRs, unreconciled |
...
discovery, survival, camp, puzzles, story, quests, weather, audio. No real player controller. No
scenes: `.unity` files must be authored in the editor per `Assets/Scenes/README.md`.

## 2. Verification status — read this before trusting anything

| Check | Result |
|---|---|
| `ci/validate-structure.py` | **exit 0** — 81 files, 99 public types, 0 errors, 47 warnings |
...
- Scenes do not exist. The project cannot run until they are authored in the editor.

## 6. How to get it running

`docs/dev-setup.md` — clean machine to running build. Summary: Unity Hub → install
`6000.6.0f1` → *Add project from disk* → **author the four scenes per `Assets/Scenes/README.md`**
→ Game view 390×844 portrait → open `Bootstrap.unity` → Play.
