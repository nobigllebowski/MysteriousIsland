...
| Tests (375 EditMode cases, 30 PlayMode) | 24 | 8,361 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 405 tests written (375 EditMode + 30 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
