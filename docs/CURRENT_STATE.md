...
| Tests (387 EditMode cases, 35 PlayMode) | 27 | 9,278 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 422 tests written (387 EditMode + 35 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
