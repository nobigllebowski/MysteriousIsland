...
| Tests (388 EditMode cases, 35 PlayMode) | 27 | 9,300 | Written, **never executed** |
...
| **Tests** | **UNCONFIRMED — 423 tests written (388 EditMode + 35 PlayMode), 0 executed.** The PlayMode suite self-skips without the scenes, so *ignored* must never be read as *passed*. |
